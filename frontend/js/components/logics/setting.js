...
function validateProfile() {
    ...
    return [...profileWrap.querySelectorAll("input")].every(validateInput);
}
...
function updateSaveState() {
    ...
    const canSave = validateProfile() && hasProfileChanged();
...
function attachValidation() {
    ...
    genderOptions?.addEventListener("mousedown", (e) => {
        if (!e.target.classList.contains("gender-item") || !genderInput) return;
...
async function saveProfile() {
    const profileWrap = getProfileWrap();
    const saveBtn = profileWrap?.querySelector(".save-info__btn");
    if (!saveBtn || !saveBtn.classList.contains("active")) return;
